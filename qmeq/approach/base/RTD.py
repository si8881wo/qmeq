...
class ApproachPyRTD(Approach):
    ...
    def prepare_arrays(self):
        ...
        self.paulifct_eps = np.zeros((nleads, ndm1, 2), dtype=doublenp) #simon
        ...
        self.Lpm = np.zeros((3*5, kern_size, kern_size), dtype = complexnp) #simon
        self.kernel_handler.set_lpm(self.Lpm) #simon
        self.current_noise = np.zeros(2, dtype = complexnp) #simon
    ...
    def generate_kern(self):
        ...
        for bcharge in range(ncharge):
            for b in statesdm[bcharge]:
                ...
                self.generate_row_1st_order_kernel_lpm(b, bcharge, 1e-10)#simon
    ...
    def generate_current_noise(self): #simon
        ...
        # pseudoinverse
        eps=1e-10
        R   = Q @ np.linalg.inv(eps*np.eye(np.size(P)) + Jdottemp) @ Q 
